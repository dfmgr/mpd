...
# @@File             :  install.sh
# @@Description      :  Install configurations for mpd
...
GLOBAL_OS_PACKAGES="mpd mpv ncmpcpp timidity "
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Run custom actions
__app_is_running mpd && MPD_IS_RUNNING="true"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__run_pre_install() {
  ...
  __kill mpd || true
  __service_is_active mpd.socket && sudo systemct disable --now mpd.socket &>/dev/null
  __service_is_active mpd.service && sudo systemct disable --now mpd.service &>/dev/null
  return $getRunStatus
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__run_post_install() {
  local getRunStatus=0
  __mkdir "$HOME/.config/ncmpcpp"
  __mkdir "$HOME/.config/mpd/playlists"
  __symlink "$HOME/.config/mpd/ncmpcpp.conf" "$HOME/.config/ncmpcpp/config"
  __replace_all "6600" "${MPDSERVER_PORT:-6600}" "$APPDIR"
  __replace_all "localhost" "${MPDSERVER:-localhost}" "$APPDIR"
  touch "$HOME/.config/mpd/tag_cache"
  __app_is_running mpd || __silent_start mpd
  return $getRunStatus
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
